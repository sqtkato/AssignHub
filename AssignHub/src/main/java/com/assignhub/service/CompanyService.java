...
public class CompanyService {
	 ...
	 * @return 企業エンティティのリスト
	 */
	public List<Company> findAll(String companySearch,String companyTelSearch ,String sort, String order) {
		return companyMapper.findAll(companySearch,companyTelSearch ,sort, order);
	}
	 ...
	 * @return 重複していればtrue
	 */
	public boolean isTelDuplicate(String companyTel,Integer companyId ) {
		return companyMapper.existsByCompTel(companyTel, companyId);
	}
			...
			while ((line = br.readLine()) != null) {
				...
				String foundedYear = cols[3].trim();
				 ...
				 String employeeCount = cols[4].trim();
					...
					// 登録上限チェック
		            if (!hasError && (currentCount + result.successCount + 1) > 500) {
